...
void desenhaCor(PNo raiz,int nivel,int deslocamento, int x_pai,int destaque) {
	...
	if (raiz != NULL) {
		...
		if(raiz->irmao) desenhaCor(raiz->irmao,nivel,(raiz->tamanho_total)+deslocamento,x_pai,destaque); /* chama mesma função para desenhar o irmão desse nó */
		if(raiz->filho) desenhaCor(raiz->filho,nivel+1,deslocamento,x+(nDigitos(raiz->chave)/2),destaque); /* chama a mesma função para desenhar o filho desse nó */
	}
}
